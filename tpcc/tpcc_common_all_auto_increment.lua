...
-- STOCK table
   ...
   con:bulk_insert_init("INSERT INTO item" .. i .." (id, i_id, i_im_id, i_name, i_price, i_data) values")
   for j = 1 , MAXITEMS do
      ...
      query = string.format([[(null, %d,%d,'%s',%f,'%s')]],
   ...
   con:bulk_insert_done()
    ...
    print(string.format("Adding indexes %d ... \n", i))
  	con:query("CREATE INDEX idx_customer_1"..i.." ON customer"..i.." (c_w_id, c_d_id, c_id)")
    con:query("CREATE INDEX idx_customer_2"..i.." ON customer"..i.." (c_w_id,c_d_id,c_last,c_first)")
    con:query("CREATE INDEX idx_orders_1"..i.." ON orders"..i.." (o_w_id,o_d_id,o_id)")
    con:query("CREATE INDEX idx_orders_2"..i.." ON orders"..i.." (o_w_id,o_d_id,o_c_id,o_id)")
    con:query("CREATE INDEX fkey_stock_1"..i.." ON stock"..i.." (s_w_id, s_i_id)")
    ...
    con:query("CREATE INDEX idx_order_line"..i.." ON order_line"..i.." (ol_w_id, ol_d_id, ol_o_id, ol_number)")
    ...
    con:query("CREATE INDEX idx_new_orders"..i.." ON new_orders"..i.." (no_w_id, no_d_id, no_o_id)")
    con:query("CREATE INDEX idx_district"..i.." ON district"..i.." (d_w_id, d_id)")
    con:query("CREATE INDEX idx_item"..i.." ON item"..i.." (i_id)")
    con:query("CREATE INDEX idx_warehouse"..i.." ON warehouse"..i.." (w_id)")
...
function load_tables(drv, con, warehouse_num)
   ...
   print(string.format("loading tables: %d for warehouse: %d\n", table_num, warehouse_num))
	...
	" (id, w_id, w_name, w_street_1, w_street_2, w_city, w_state, w_zip, w_tax, w_ytd) values")

    query = string.format([[(null, %d, '%s','%s','%s', '%s', '%s', '%s', %f,300000)]],
	...
	" (id, d_id, d_w_id, d_name, d_street_1, d_street_2, d_city, d_state, d_zip, d_tax, d_ytd, d_next_o_id) values")

   for d_id = 1 , DIST_PER_WARE do
 
      query = string.format([[(null, %d, %d, '%s','%s','%s', '%s', '%s', '%s', %f,30000,3001)]],
...
-- CUSTOMER TABLE
   ...
   con:bulk_insert_init("INSERT INTO customer" .. table_num .. [[
	  (id, c_id, c_d_id, c_w_id, c_first, c_middle, c_last, c_street_1, c_street_2, c_city, c_state, c_zip,
   ...
   for c_id = 1 , CUST_PER_DIST do
	...
	if c_id <= 1000 then
      ...
      query = string.format([[(null, %d, %d, %d, '%s','OE','%s','%s', '%s', '%s', '%s', '%s','%s',NOW(),'%s',50000,%f,-10,10,1,0,'%s' )]],
...
-- HISTORY TABLE
   ...
   con:bulk_insert_init("INSERT INTO history" .. table_num .. [[
	  (id, h_c_id, h_c_d_id, h_c_w_id, h_d_id, h_w_id, h_date, h_amount, h_data) values]])

   for d_id = 1 , DIST_PER_WARE do
   for c_id = 1 , CUST_PER_DIST do
 
      query = string.format([[(null, %d, %d, %d, %d, %d, NOW(), 10, '%s' )]],
   ...
   con:bulk_insert_init("INSERT INTO orders" .. table_num .. [[
	  (id, o_id, o_d_id, o_w_id, o_c_id, o_entry_d, o_carrier_id, o_ol_cnt, o_all_local) values]])
...
-- 3,000 rows in the ORDER table with
   a_counts[warehouse_num][d_id][o_id] = sysbench.rand.uniform(5,15)
      ...
      query = string.format([[(null, %d, %d, %d, %d, NOW(), %s, %d, 1 )]],
...
-- STOCK table
   ...
   con:bulk_insert_init("INSERT INTO stock" .. table_num .. 
	" (id, s_i_id, s_w_id, s_quantity, s_dist_01, s_dist_02, s_dist_03, s_dist_04, s_dist_05, s_dist_06, "..
        " s_dist_07, s_dist_08, s_dist_09, s_dist_10, s_ytd, s_order_cnt, s_remote_cnt, s_data) values")

   for s_id = 1 , 100000 do
 
      query = string.format([[(null, %d, %d, %d,'%s','%s','%s','%s','%s','%s','%s','%s','%s','%s',0,0,0,'%s')]],
   ...
   con:bulk_insert_init("INSERT INTO order_line" .. table_num .. [[
	  (id, ol_o_id, ol_d_id, ol_w_id, ol_number, ol_i_id, ol_supply_w_id, ol_delivery_d,
   ...
   for ol_id = 1, a_counts[warehouse_num][d_id][o_id] do 
      ...
      query = string.format([[(null, %d, %d, %d, %d, %d, %d, %s, 5, %f, '%s' )]],
